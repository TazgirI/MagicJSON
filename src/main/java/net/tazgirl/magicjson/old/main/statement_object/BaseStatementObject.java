...
import net.tazgirl.magicjson.old.main.statement_object.interface_categories.BaseStatementInterface;
...
public abstract class BaseStatementObject implements BaseStatementInterface
{
    ...
    // Warning, if used with an incorrect interface/expectedTypePair
    public <T> T cleanResolveOrResolvesObject(BaseStatementObject statementObject, Class<? extends BaseStatementInterface> secondaryCheck, Class<T> expectedType)
    {
        Boolean isCheckType = secondaryCheck.isInstance(statementObject);
        Boolean isAnyType = statementObject instanceof ResolvesObject;

        if(!(isCheckType || isAnyType)){return null;}

        Object result = statementObject.Resolve();

        if(expectedType.isInstance(result)){return (T) result;}
