...
public class LivingDamageEventStatementObject extends EventHook
{
    State state = State.PRE;

    public LivingDamageEventStatementObject(StatementHolder holder)
    {
        super(holder);
    }
    ...
    {
        if(hookArguments.get(Constants.eventParamName).resolve() instanceof LivingDamageEvent.Post postEvent)
        {
    ...
    {
        if(hookArguments.get(Constants.eventParamName).resolve() instanceof LivingDamageEvent.Pre preEvent)
        {
            return switch(function)
            {
    ...
    {
        String stateName = state == null ? State.UNINITATED.name().toLowerCase() : state.name().toLowerCase();
        return "living_damage_" + stateName+ "( " + arguments + " )";
    }
